...
bool all_repeating_groups(string& n, int m) {
    string sn = n.substr(0, m);
    int i = -1;
    for (i = m; i < n.length(); i += m) {
        string sm = n.substr(i, m);
        if (sn != sm) {
            return false;
        }
    }
